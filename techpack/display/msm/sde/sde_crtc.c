						...
						int cnt)
{
	...
	if (dimlayer_hbm || dimlayer_bl) {
		...
		if (aod_index >= 0) {
			...
			if (((fp_index >= 0) || (fppressed_index < 0)) &&
			    ((panel_power_mode == SDE_MODE_DPMS_LP1) ||
			     (panel_power_mode == SDE_MODE_DPMS_LP2))) {
		...
		SDE_DEBUG(
			"aod_index = %d, fp_index= %d, fppressed_index = %d, fp_mode=%d, panel_power_mode = %d, bl=%d\n",
			aod_index, fp_index, fppressed_index, fp_mode,
			panel_power_mode, oplus_get_panel_brightness());
