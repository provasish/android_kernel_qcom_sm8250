		       ...
		       struct panel_info *panel_data)
{
	...
	if (prj_id == 21027) {
		...
		panel_data->firmware_headfile.firmware_data =
			FW_21027_NT36523_BOE;
		panel_data->firmware_headfile.firmware_size =
			sizeof(FW_21027_NT36523_BOE);
		snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
			 "tp/%d/FW_%s_%s.bin", prj_id, panel_data->chip_name,
			 vendor);
...
int reconfig_power_control(struct touchpanel_data *ts)
{
	...
	if ((prj_id == 20135 || prj_id == 20137 || prj_id == 20139 ||
	     prj_id == 20235) &&
	    !strstr(saved_command_line,
		    "20135samsung_amb655xl08_1080_2400_cmd_dvt")) {
		...
		if (IS_ERR_OR_NULL(ts->hw_res.vdd_2v8)) {
			...
			if (regulator_count_voltages(ts->hw_res.vdd_2v8) > 0) {
				...
				ret = regulator_set_load(ts->hw_res.vdd_2v8,
							 200000);
				if (ret < 0) {
					dev_err(ts->dev,
